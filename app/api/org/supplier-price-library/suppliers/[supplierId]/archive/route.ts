...
export async function POST(_request: Request, context: RouteContext<"/api/org/supplier-price-library/suppliers/[supplierId]/archive">) {
  const auth = await requireSupplierPriceSession("delete"); if ("response" in auth) return auth.response; const { supplierId } = await context.params; if (!validUuid(supplierId)) return jsonError("Supplier not found", 404);
