...
export async function PATCH(request: Request, context: RouteContext<"/api/org/supplier-price-library/categories/[categoryId]">) {
  const auth = await requireSupplierPriceSession("admin"); if ("response" in auth) return auth.response; const { categoryId } = await context.params; if (!validUuid(categoryId)) return jsonError("Category not found", 404);
  ...
  if (error) { if (isDuplicateError(error)) return jsonError("A category with this name already exists in your organization.", 409); return jsonError("Unable to rename category", 500); } if (!data) return jsonError("Category not found", 404); return Response.json({ category: data });
}
