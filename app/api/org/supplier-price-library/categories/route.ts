...
export async function GET(request: Request) {
  ...
  const params = new URL(request.url).searchParams; const search = safeSearch(params.get("search") ?? ""); const status = params.get("status") ?? "active";
  ...
  const { data, error } = await query.order("category_name", { ascending: true }); if (error) { logDatabaseError("Unable to fetch categories", error); return jsonError("Unable to fetch categories", 500); }
...
export async function POST(request: Request) {
  const auth = await requireSupplierPriceSession("admin"); if ("response" in auth) return auth.response; let body: { category_name?: unknown }; try { body = await request.json(); } catch { return jsonError("Invalid request body", 400); }
  ...
  if (error) { if (isDuplicateError(error)) return jsonError("A category with this name already exists in your organization.", 409); logDatabaseError("Unable to create category", error); return jsonError("Unable to create category", 500); }
