...
function uniqueIds(value: unknown) {
  ...
  return Array.from(new Set(value.filter((item): item is string =>
    typeof item === "string" && /^[0-9a-f-]{36}$/i.test(item),
  )));
}

function correctionErrorMessage(message: string | undefined) {
  const allowed = [
    ...
    "Job not found",
    "Only a Work Completed job can be corrected",
    "Only the latest completion certificate can be corrected",
    "Source completion certificate not found",
    "A reopened completion certificate cannot be corrected",
    "One or more selected Technicians are invalid or inactive",
    "The source certificate has no Work Order scope",
  ];
  return allowed.find((value) => message?.includes(value)) ?? "Unable to prepare the completion correction";
}
...
export async function POST(
  ...
  const { data: sourceCompletion, error: sourceError } = await admin
    .from("job_work_completions")
    .select("certificate_storage_path,generation_status")
    .eq("id", completionId)
    .eq("job_id", jobId)
    .eq("org_id", session.org_id)
    .maybeSingle();
  if (sourceError) return jsonError("Unable to validate the existing certificate", 500, "SOURCE_VALIDATION_FAILED");
  if (!sourceCompletion || sourceCompletion.generation_status !== "generated") {
    return jsonError("Completion certificate not found", 404, "SOURCE_NOT_FOUND");
  }
  const { data: draftValue, error: draftError } = await admin.rpc(
    "create_job_work_completion_correction_draft",
    {
      p_org_id: session.org_id,
      p_job_id: jobId,
      p_source_completion_id: completionId,
  ...
  if (draftError) {
    console.error("Unable to create completion correction draft", { code: draftError.code, message: draftError.message });
    if (draftError.code === "PGRST202" || draftError.message.includes("create_job_work_completion_correction_draft")) {
      return jsonError("The completion correction database migration has not been applied", 503, "MIGRATION_REQUIRED");
    }
    return jsonError(correctionErrorMessage(draftError.message), 409, "CORRECTION_REJECTED");
  }
  const draft = draftValue as { id?: string; certificate_number?: string; revision_number?: number } | null;
  if (!draft?.id || !draft.certificate_number) {
    return jsonError("Unable to reserve a corrected certificate", 500, "CERTIFICATE_RESERVATION_FAILED");
  }

  const markFailed = async () => {
    await admin.from("job_work_completions").update({ generation_status: "failed" })
      .eq("id", draft.id).eq("org_id", session.org_id).eq("generation_status", "generating");
  };
  const pdfResult = await getWorkCompletionPdfData(admin, session.org_id, jobId, draft.id);
  if (pdfResult.error || !pdfResult.data) {
    await markFailed();
    return jsonError("Unable to collect corrected acknowledgement information", 500, "CERTIFICATE_DATA_FAILED");
  }
  ...
  try {
    ...
    console.error("Unable to render corrected Work Completion Acknowledgement", { completionId: draft.id, error });
    await markFailed();
    return jsonError("Unable to generate the corrected Work Completion Acknowledgement", 500, "CERTIFICATE_RENDER_FAILED");
  }
  ...
  const revision = Number(draft.revision_number ?? 1);
  const fileName = `${draft.certificate_number}-revision-${revision}.pdf`;
  const filePath = `${session.org_id}/jobs/${jobId}/work-completions/${draft.id}.pdf`;
  const { error: uploadError } = await admin.storage.from(bucket).upload(filePath, pdf, {
    contentType: "application/pdf",
    upsert: false,
  });
  if (uploadError) {
    await markFailed();
    return jsonError("Unable to store the corrected Work Completion Acknowledgement", 500, "CERTIFICATE_UPLOAD_FAILED");
  }

  const generatedAt = new Date().toISOString();
  const { error: finalizeError } = await admin.rpc("finalize_job_work_completion_correction", {
    p_org_id: session.org_id,
    p_job_id: jobId,
    p_source_completion_id: completionId,
  ...
  if (finalizeError) {
    ...
    await markFailed();
    return jsonError("The completion changed before the correction could be finalized", 409, "CORRECTION_CONFLICT");
  }

  const expectedSourcePrefix = `${session.org_id}/jobs/${jobId}/work-completions/`;
  if (sourceCompletion.certificate_storage_path?.startsWith(expectedSourcePrefix)
      && sourceCompletion.certificate_storage_path !== filePath) {
    const { error: removeError } = await admin.storage.from(bucket)
      .remove([sourceCompletion.certificate_storage_path]);
    if (removeError) {
      console.error("Corrected certificate saved but superseded PDF cleanup failed", {
        jobId,
        completionId,
