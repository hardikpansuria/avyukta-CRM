...
export async function POST(request: Request) {
  ...
  const invoiceRequestId = value("invoice_request_id");
  ...
  if (!(fileEntry instanceof File) || fileEntry.size === 0) {
    return jsonError("An invoice PDF copy is required", 400);
  }
  ...
  if (invoiceRequestId) {
    const { data: invoiceRequest, error: requestError } = await admin
      .from("invoice_requests")
      .select("id,job_id,purchase_order_id,status,invoice_id")
      .eq("id", invoiceRequestId)
      .eq("org_id", session.org_id)
      .maybeSingle();
    if (requestError) return jsonError("Unable to validate invoice request", 500);
    if (!invoiceRequest) return jsonError("Invoice request not found", 404);
    if (
      invoiceRequest.job_id !== jobId ||
      invoiceRequest.purchase_order_id !== allocation.purchase_order_id
    ) {
      return jsonError("Invoice does not match the linked request", 400);
    }
    if (invoiceRequest.status !== "under_review" || invoiceRequest.invoice_id) {
      return jsonError("Invoice request is not ready for invoice upload", 409);
    }
  }
  ...
  if (upload.error) {
    await admin.from("job_invoices").delete().eq("id", invoice.id).eq("org_id", session.org_id);
    return jsonError("Unable to upload invoice PDF copy", 500);
  }

  if (invoiceRequestId) {
    const { data: linkedRequest, error: linkError } = await admin
      .from("invoice_requests")
      .update({
    ...
    if (linkError || !linkedRequest) {
      return jsonError("Invoice uploaded, but the request could not be linked", 500);
    }
    if (linkedRequest.requested_by) {
      await admin.from("crm_notifications").insert({
        org_id: session.org_id,
        user_id: linkedRequest.requested_by,
        kind: "invoice_created",
        title: `Invoice ${invoice.invoice_number} has been uploaded`,
        message: `Request IR-${String(linkedRequest.request_number).padStart(3, "0")}`,
        href: `/dashboard/invoices/${invoice.id}`,
      });
    }
  }
  return NextResponse.json(
    { invoice, document_warning: null },
    { status: 201 },
  );
}
