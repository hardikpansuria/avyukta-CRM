...
function Filter({ value, onChange, all, options, allValue = "all" }: { value: string; onChange: (value: string) => void; all: string; options: string[][]; allValue?: string }) { const selectedLabel = value === allValue ? all : options.find(([option]) => option === value)?.[1] ?? all; return <Select value={value} onValueChange={(next) => onChange(String(next))}><SelectTrigger className="w-full"><SelectValue>{selectedLabel}</SelectValue></SelectTrigger><SelectContent><SelectItem value={allValue}>{all}</SelectItem>{options.map(([option, label]) => <SelectItem key={option} value={option}>{label}</SelectItem>)}</SelectContent></Select>; }
